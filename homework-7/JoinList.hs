...
data JoinList m a = Empty
                  | Single m a
                  | Append m (JoinListBasic a) (JoinListBasic a)
...
indexJ idx (Append siz jl1 jl2)
  ...
  | otherwise   = indexing idx jl1
    where size1 = getSize . tag jl1
...
takeJ n jl@(Append siz jl1 jl2)
  | n >= siz = jl
  | n >= size1 = jl1 +++ takeJ (idx - size1) jl2
